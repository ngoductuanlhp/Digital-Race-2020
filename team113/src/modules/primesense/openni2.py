...
#The types of streams: use in (device).get_sensor_info() to get supported video modes
...
#use following in (stream).set_video_mode()
...
'''
Use this class to: 
    *find out what video modes are available:
        *resolution, frame rate, pixel type
'''
...
Use this class to:
    ...
    *Get streams (rgb, depth, ir)
    *get the above sensor info class
    *sync the streams
'''
class Device(HandleObject):
    ...
    #this will get you a camera if there is one that has openni drivers
    @classmethod
    def open_any(cls):
        return cls(None)
